...
const PostEditorGallery = ({items, maxHeight, maxWidth, onRemove}: PostEditorGalleryProps) => {
    ...
    return (
        <FlatList
            data={resizeImages()}
            renderItem={({item}) => <GalleryItem item={item} onRemove={onRemove}/>}
            horizontal
    ...
    return (
        <View style={{position: "relative", marginRight: 12}}>
            <Video
                ...
                isLooping
                onPlaybackStatusUpdate={status => setMuted((status as AVPlaybackStatusSuccess).isMuted)}
            ...
            {onRemove && (
                <TouchableOpacity onPress={event => onRemove(item)} style={{
                    width: 32,
                    height: 32,
                    position: "absolute",
                    backgroundColor: 'rgba(0, 0, 0, 0.4)',
...
const ImageItem = ({onRemove, onTouch, item}: ImageItemProps) => {
    ...
    return (
        <View style={{position: "relative", marginRight: 12}}>
            <Image source={item.uri} onProgress={event => onTouch && onTouch(item)} style={{
                borderRadius: 10,
                borderColor: "#eee",
                borderWidth: 1,
                width: item.width,
